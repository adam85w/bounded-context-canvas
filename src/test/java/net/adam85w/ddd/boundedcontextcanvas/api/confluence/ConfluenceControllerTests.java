package net.adam85w.ddd.boundedcontextcanvas.api.confluence;
...
public class ConfluenceControllerTests {

    private final String ENDPOINT = "/api/bounded-context-canvas/confluence";
    ...
    public void unrecognizedTemplateNameShouldReturnError() throws IOException {
        ...
        ErrorResponse response = objectMapper.readValue(responseEntity.getBody(), ErrorResponse.class);
        Assertions.assertThat(response.getMessage()).isEqualTo("Template name is incorrect or does not support the CONFLUENCE type.");
    }
